...
import { NgbDate, NgbModule, NgbDatepicker, NgbDateAdapter, ModalDismissReasons, NgbModal, NgbModalOptions } from '@ng-bootstrap/ng-bootstrap';
...
import { input } from '../../UserManagment/admin-rolemaster/admin-rolemaster.component';
...
@Component({
  selector: 'app-time-line-invoice',
  templateUrl: './time-line-invoice.component.html',
  styleUrls: ['./time-line-invoice.component.scss']
})
export class TimeLineInvoiceComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.InvoicetimelineData(ExportArrayInput1)
  ...
  InvoicetimelineData(ListInput: Input) {
    // this.loader.open();
    ...
    this.MISService.Invoicetimeline(ListInput).subscribe(
  ...
  Search() {
    ...
    if (this.itemForm.value.days !== null && this.itemForm.value.days) {
      if (this.itemForm.value.days !== "" && this.itemForm.value.days !== "") {
        // if (Number(this.itemForm.value.from_percentage) > Number(this.itemForm.value.to_percentage)) {
          Swal.fire(" Please Select From percentage either  Please Select to percentage  '");
          return false
        // }
      }
    }
    ...
    // if (this.itemForm.value.days == null || this.itemForm.value.days == "" && this.itemForm.value.days !== null) {
    //   Swal.fire('Select to percentage');
    //   const ListInput: Input = {} as Input;
    ...
    this.itemForm.value.account_name = this.account_name
